...
Model ModelReader::ReadModelObjData(string filename)
{
	m_vertices.clear();
	...
	for (int i = 0; i < m_vertices.size(); i) {
		Vertex v(
			glm::vec3(m_vertices.at(i), m_vertices.at(i + 1), m_vertices.at(i + 2)),
			glm::vec3(1.0f, 1.0f, 1.0f)
		);
		i += 3;
		model.vertices.push_back(v);	//Add vertex
	}

	for (int i = 0; i < m_faceVertexIndices.size(); i++) {
		model.indices.push_back(m_faceVertexIndices.at(i));
	}
	model.set();
